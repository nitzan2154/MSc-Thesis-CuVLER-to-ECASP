...
% BEGIN PAPER FIGURE: main.tex -- ECASP architecture

\begin{figure*}[!htbp]
\centering
\includesvg[width=\textwidth]{graphics/images/ECASP_full_model_final_final_best}

\caption{ECASP architecture. The model takes multi-view binary masks and camera calibration as input.
Each mask is cropped, resized, and encoded by a shared DINOv3 backbone.
Pl\"ucker-ray positional encoding is then added to the extracted feature maps before multi-view fusion.
The per-view features are enriched symmetrically through epipolar cross-view attention and then aggregated.
The resulting multi-view representation is passed to a hybrid pose head that predicts rotation and combines triangulated and residual translation estimates.}
\label{fig:architecture}
\end{figure*}

% END PAPER FIGURE: ECASP architecture
...
% BEGIN PAPER FIGURE: main.tex -- Epipolar cross-view attention

\begin{figure}[!htbp]
\centering
\includesvg[width=\linewidth]{graphics/images/cross_attention_diagram}

\caption{Epipolar cross-view attention. For each query token, ECASP samples \(S\) locations along the calibrated epipolar line in each source view. The sampled features from all \(N-1\) views form the keys and values for multi-head cross-attention.}
\label{fig:epipolar_attention}
\end{figure}

% END PAPER FIGURE: Epipolar cross-view attention
...
% BEGIN PAPER FIGURE: main.tex -- Hybrid pose head

\begin{figure*}[!htbp]
\centering
\includesvg[width=\textwidth]{graphics/images/_hybrid_pose_estimation_final_better}

\caption{Hybrid pose head. Rotation is predicted from aggregated and view-specific features, while translation combines DLT triangulation of the predicted CAD-origin projections with a confidence-gated residual correction. Loss-only auxiliary heads are used during training.}

\label{fig:hybrid_pose_head}
\end{figure*}

% END PAPER FIGURE: Hybrid pose head
